...
export class PullRequest {
	...
	private head: Fork;
	private base: Fork;
	private taskmaster: ChildProcess.Taskmaster;
	...
	constructor(request: Github.PullRequest, private token: string, path: string, jobQueueHandler: ChildProcess.JobQueueHandler, parentQueue: PullRequestQueue, statusTargetUrl: string, private branches: any) {
		...
		this.taskmaster = new ChildProcess.Taskmaster(path, token, this.branches, this.number, user, this.repositoryName, organization, upstreamBranch, branch, jobQueueHandler, this.updateExecutionResults.bind(this));
	...
	processPullRequest() {
		...
		try {
			...
			GithubApi.PullRequest.updateStatus("pending", "PRInt is working on your pull request.", this.statusesUrl, this.statusTargetUrl);
		}
		catch (error) {
			...
			GithubApi.PullRequest.updateStatus("error", "There was an error when PRInt was processing your pull request.", this.statusesUrl, this.statusTargetUrl);
		}
	}
	updateExecutionResults(executionResults: ChildProcess.ExecutionResult[], allJobsComplete: boolean) {
		...
		if (allJobsComplete) {
			...
			if (status)
				GithubApi.PullRequest.updateStatus("success", "PRInt succeeded. You are great!", this.statusesUrl, this.statusTargetUrl);
			else
				GithubApi.PullRequest.updateStatus("failure", "PRInt failed. This is not good!", this.statusesUrl, this.statusTargetUrl);
		}
	}
